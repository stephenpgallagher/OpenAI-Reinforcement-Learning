    ...
    **YOU NEED TO IMPLEMENT FUNCTIONS IN THIS CLASS**
    ...
    def schedule_hyperparameters(self, timestep: int, max_timestep: int):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        """
        ### PUT YOUR CODE HERE ###
        self.epsilon = 1 - (timestep/(0.5*max_timestep))
        if self.epsilon<0.01:
            self.epsilon = 0.01
        
        if timestep/self.target_update_freq >= self.update_counter+1:
    ...
    def act(self, obs: np.ndarray, explore: bool):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        with torch.no_grad():
            action_vals = self.critics_net(obs)
    ...
    def update(self, batch: Transition) -> Dict[str, float]:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        Q_targets_next = self.critics_target(next_states).max(1)[0][:,None]
        # Compute Q targets for current states 
        Q_targets = rewards + (self.gamma * Q_targets_next * (1 - dones))

        # Get expected Q values from local model
        Q_expected = self.critics_net(obs).gather(1, actions)

        # Compute loss
        q_loss = torch.nn.functional.mse_loss(Q_expected,Q_targets)
    ...
    ** YOU NEED TO IMPLEMENT THE FUNCTIONS IN THIS CLASS **
    ...
    def __init__(
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        ###  Building networks and optimisers (DO NOT CHANGE THE OUTPUT ACTIVATION OF THIS POLICY)
        ...
        ### Write any extra hyperparameters needed here
        self.learning_rate = learning_rate
        self.gamma = gamma
        ###
    ...
    def update(
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        R = 0
        temp_list = []
        for z in zip(observations,actions,rewards):
            temp_list.append(z)
        for o,a,r in temp_list[::-1]:
            R = r + self.gamma*R
            o = torch.from_numpy(o).type(torch.float32)
            p_loss = p_loss - R*torch.log(self.policy(o)[a])
