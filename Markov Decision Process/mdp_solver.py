...
class ValueIteration(MDPSolver):
    ...
    def _calc_value_func(self, theta: float) -> np.ndarray:
        ...
        **DO NOT ALTER THE MDP HERE**
        ...
        # While the improvement is above theta
        while True:
            ...
            for s in range(self.state_dim):
                ...
                # Forward one step to find optimal action
                A = np.zeros(self.action_dim)
                for a in range(self.action_dim):
                    
                    # Bellman equation
                    V_next_state = mdp.P[s, a] * (mdp.R[s, a] + self.gamma * V)
                    A[a] = sum(V_next_state)
                
                # Updating action value
                V[s] = max(A)
                
                # Maximum difference over the states
                delta = max(delta, np.abs(v - V[s]))
            
            # Stopping if improvement is below theta
            if delta < theta:
                break
...
class PolicyIteration(MDPSolver):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        while True:
            ...
            for s in range(self.state_dim):
                ...
                # Bellman equation
                V_next_state = mdp.P[s, np.argmax(policy[s])] * (mdp.R[s, np.argmax(policy[s])] + self.gamma * V)
                
                # Summing values for next state
                V[s] = sum(V_next_state)
    ...
    def _policy_improvement(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        # Initializing policy and value function
        policy = np.zeros([self.state_dim, self.action_dim])
        V = np.zeros([self.state_dim])
        ...
        while True:
            ...
            for s in range(self.state_dim):
                ...
                for a, prob in enumerate(policy[s]):
                    ...
                    # Bellman equation
                    V_next_state = mdp.P[s, a] * (mdp.R[s, a] + self.gamma * V)
                    temp_a[a] = sum(V_next_state)
                    
                # Optimal action
                best_a = np.argmax(temp_a)
                
                # Deterministic setting
                policy[s] = 0
                policy[s, best_a] = 1
                
                # Checking policy stability
                if best_a != new_a:
                    policy_stable = False
            
            # Stopping when policy is stable
            if policy_stable:
                break
